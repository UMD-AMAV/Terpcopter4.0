...
fprintf('Distance Error %f meters\n', distanceErrorMeters);
if (distanceErrorMeters <= outerAttitudeDeadbandMeters)
    [yawStickCmd, uStickCmd, vStickCmd] = waypointForwardCrabController(curTime, yawDeg, x_d, x, y_d, y);
else
    [yawStickCmd, uStickCmd, vStickCmd, yawControl] = waypointPointAndMoveForwardController(yawControl, curTime, yawDeg, x_d, x, y_d, y);
end
